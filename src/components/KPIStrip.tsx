...
const KPICard: React.FC<{ item: KPIItem }> = React.memo(({ item: k }) => (
  <motion.div
    variants={cardAnim}
    className="flex flex-col gap-0.5 rounded-[0.375rem] border border-[rgba(0,0,0,0.08)] bg-white px-3 py-2 min-w-0 transition-shadow duration-150 hover:shadow-[0_0.125rem_0.5rem_rgba(0,0,0,0.07)] cursor-default"
  >
    <div className="text-[1.25rem] font-bold leading-none tracking-tight" style={{ color: k.color, fontFamily: 'var(--font-display)' }}>
      {k.value.toLocaleString()}
    </div>
    <div className="text-[0.5625rem] font-bold uppercase tracking-[0.0375rem] text-[#9B9992] truncate mt-0.5">{k.label}</div>
    <div className="text-[0.625rem] text-[#C4C2BB] truncate">{k.sub}</div>
  </motion.div>
));
