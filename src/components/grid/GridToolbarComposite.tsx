...
/**
 * Composite Toolbar: combines MUI built-in toolbar buttons
 * with custom shadcn filter chips and search input.
 * Follows composite pattern — each section is isolated.
 */
export const GridToolbarComposite: React.FC = () => {
  ...
  const collapseAll        = useDashboardStore(s => s.collapseAll);
  ...
  const totalOpps = paginatedStudents.reduce((a, s) => a + s.opportunities.length, 0);

  return (
    <div className="flex flex-col border-b border-[rgba(0,0,0,0.08)] bg-[#FAFAF9]">
      {/* Row 1: Search + MUI toolbar buttons + stats */}
      <div className="flex items-center gap-2 px-3 py-2 flex-wrap">
        ...
        {/* MUI built-in buttons — composite slot usage */}
        ...
        {/* Expand / Collapse */}
        <Button variant="ghost" size="xs" onClick={expandAll} title="Expand all students">
          <ExpandAllIcon />
          Expand All
        </Button>
        <Button variant="ghost" size="xs" onClick={collapseAll} title="Collapse all students">
          <CollapseAllIcon />
          Collapse All
        </Button>
        ...
        <div className="ml-auto text-[10px] text-[#9B9992] font-mono tabular-nums whitespace-nowrap">
          <span className="font-semibold text-[#5C5B57]">{totalRows.toLocaleString()}</span> students
          &nbsp;·&nbsp;
          <span className="font-semibold text-[#5C5B57]">{totalOpps.toLocaleString()}</span> opps
        </div>
      </div>

      {/* Row 2: Filter chips */}
      <div className="flex items-center gap-1 px-3 pb-2 flex-wrap">
        <span className="text-[9px] font-bold uppercase tracking-[0.6px] text-[#C4C2BB] mr-1">Filter:</span>
        {FILTERS.map(f => (
          <motion.button
            key={f.mode}
