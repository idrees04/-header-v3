...
export type GridRow = {
  id: string;
  kind: "student" | "opportunity" | "opp_header";
  path: (string | number)[];
  ...
  // opportunity fields...
  opp_id: string | number | null;
  opp_name: string;
  opp_institute_name?: string;
  opp_course_level?: string | number | null;
  opp_counselor?: string | null;
  opp_adm_officer?: string | null;
  opp_office?: string | null;
  opp_subagent?: string | null;
...
export function buildGridRows(students: Student[]): GridRow[] {
  ...
  for (const s of students) {
    ...
    const studentRow: GridRow = {
      ...
      // child index not used for student
      opp_row_index: undefined,
    };

    rows.push(studentRow);
    if (s.opportunities.length > 0) {
      rows.push({
        id: `opp-header::${s.std_id}`,
        kind: "opp_header",
        path: [s.std_id],
