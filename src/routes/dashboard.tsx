...
import { ChevronDown, ChevronRight } from 'lucide-react';
import { GRID_DETAIL_PANEL_TOGGLE_COL_DEF, GridRowId } from '@mui/x-data-grid-pro';
import Tooltip from '@mui/material/Tooltip';
...
const getUniqueStudents = (data: any[]) => {
  const grouped = new Map<string, any[]>();
  data.forEach((item) => {
    if (!grouped.has(item.std_id)) grouped.set(item.std_id, []);
    grouped.get(item.std_id)!.push(item);
  });
  const uniqueStudents: any[] = [];
  grouped.forEach((records) => {
...
const pillStyle = (bg: string, color: string, width?: string): React.CSSProperties => ({
  display: 'inline-flex',
  alignItems: 'center',
  justifyContent: 'center',
  height: '24px',
...
const baseColumns: GridColDef[] = [
  ...
  // },
  {
    ...
    renderCell: (params: GridRenderCellParams) => {
      ...
      return (
        <Tooltip title={name} placement="top" arrow>
          <span style={{ display: 'flex', flexDirection: 'row', alignItems: 'start', justifyContent: 'left', textAlign: 'start', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {name}
          </span>
        </Tooltip>
      );
  ...
  { field: 'std_phone_mobile', headerName: 'Phone Number', width: 124 },
  {
    ...
    renderCell: (params: GridRenderCellParams) => {
      ...
      return (
        <Tooltip title={value} placement="top" arrow>
          <span style={pillStyle(style.bg, style.color, '74px')}>
            {value === 'female' ? 'Female' : 'Male'}
          </span>
        </Tooltip>
      );
  ...
  { field: 'std_office', headerName: 'Office', width: 70 },
  { field: 'std_subagent', headerName: 'Subagent', width: 90 },
  { field: 'std_date_entered', headerName: 'Date Entered', width: 110 },
  // { field: 'std_date_entered_year', headerName: 'Date Entered Year', width: 140 },
  // { field: 'std_date_entered_month', headerName: 'Date Entered Month', width: 100 },
  { field: 'std_email', headerName: 'Email', width: 100 },
  // { field: 'opp_id', headerName: 'Opp Id', width: 100 },
  ...
  { field: 'opp_institute_name', headerName: 'Opp Institute Name', width: 160 },
  {
    ...
    renderCell: (params: GridRenderCellParams) => {
      ...
      return (
        <Tooltip title={value} placement="top" arrow>
          <span style={pillStyle(style.bg, style.color, '130px')}>
            {value}
          </span>
        </Tooltip>
      );
  ...
  { field: 'opp_subagent', headerName: 'Opp Subagent', width: 120 },
  {
    ...
    headerName: 'Opp Sales Stage',
    width: 220,
    renderCell: (params: GridRenderCellParams) => {
      const value = params.value;
      if (!value) return '-';
      const style = salesStageColors[value] ?? { bg: '#F1EFE8', color: '#5F5E5A' };
      return (
        <Tooltip title={value} placement="top" arrow>
          <span
            style={{
...
function DashboardPage() {
  ...
  const detailColumns: GridColDef[] = useMemo(
    () => [
      ...
      {
        field: 'opp_adm_officer_uname',
        headerName: 'Admin Officer Name',
        width: 120,
        renderCell: (params: GridRenderCellParams) => {
          const value = params.value ?? '';
          const initials = value
      ...
      },
      {
        ...
        renderCell: (params: GridRenderCellParams) => {
          ...
          const style = stageColors[value] ?? { bg: '#F1EFE8', color: '#5F5E5A' };
          return (
            <Tooltip title={value} placement="top" arrow>
              <span
                style={{
  ...
  const columns: GridColDef[] = useMemo(
    () => [
      ...
      ...baseColumns.map((col) => ({
        ...col,
        valueFormatter: (value: any) => {
          if (col.field === 'std_id' && value) return String(value).split('-')[0];
          return renderDash(value);
        },
      })),
    ],
        ...
        <DataGridPro
          ...
          onRowClick={(params) => {
            ...
            setExpandedRows((prev) => {
              ...
              next.has(params.id) ? next.delete(params.id) : next.add(params.id);
              return next;
            });
          }}
          pageSizeOptions={[10, 15, 20, 25, 30]}
          ...
          sx={{
            ...
            '& .MuiDataGrid-row:nth-of-type(odd)': { backgroundColor: '#F7F6FF' },
            '& .MuiDataGrid-row--firstVisible': { backgroundColor: '#F7F6FF important' },
            ...
            '& .MuiDataGrid-row:hover .MuiDataGrid-cell': { fontSize: '13px', fontWeight: 600 },
            '& .MuiDataGrid-row:nth-of-type(odd):hover': { backgroundColor: '#ffffff !important' },
            '& .MuiDataGrid-row:nth-of-type(even):hover': { backgroundColor: '#ffffff !important' },
            '& .MuiDataGrid-row.Mui-selected, & .MuiDataGrid-row.row--expanded': {
              transform: 'scaleX(0.995) translateY(-1px)',
              boxShadow: '0 4px 16px 0 rgba(0,0,0,0.10), 0 1.5px 4px 0 rgba(0,0,0,0.07)',
              backgroundColor: '#fff',
            ...
            '& .MuiDataGrid-row.Mui-selected:hover': { backgroundColor: '#fff !important' },
            '& .MuiDataGrid-row.row--expanded': {
              transform: 'scaleX(0.995) translateY(-2px)',
              boxShadow: '0 8px 24px 0 rgba(0,0,0,0.13), 0 2px 6px 0 rgba(0,0,0,0.09)',
              backgroundColor: '#ffffff !important',
            ...
            '& .MuiDataGrid-cell': { padding: '0 8px', fontSize: '13px' },
            '& .MuiDataGrid-row.row--expanded:hover': {
              backgroundColor: '#ffffff !important',
              transform: 'scaleX(0.995) translateY(-2px)',
              boxShadow: '0 8px 24px 0 rgba(0,0,0,0.13), 0 2px 6px 0 rgba(0,0,0,0.09)',
            ...
            '& .MuiDataGrid-detailPanel': { position: 'relative' },
            '& .MuiDataGrid-detailPanel::before': {
              content: '""',
              position: 'absolute',
              left: 0,
            ...
            '& .MuiDataGrid-detailPanel .MuiDataGrid-row': { marginLeft: '50px' },
          }}
          getRowClassName={(params) => (expandedRows.has(params.id) ? 'row--expanded' : '')}
        />
      </Box>
    </AppLayout>
  );
